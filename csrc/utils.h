...
static __device__ __forceinline__ T WarpReduce(T x, Operator &op) {
    static_assert(WARPS == 32 || WARPS == 16 || WARPS == 8 || WARPS == 4 || WARPS == 2 || WARPS == 1);
...
int64_t resolve_thread_kv_page_slice_offset(
    ...
    const int global_row_offset = n_block * kBlockN + block_row_offset;
...
template <int HRatio, int GmemThreadPerRow, int RowsPerThread, int GmemElemsPerLoad>
__forceinline__ __device__
int resolve_thread_qo_slice_offset(
    const int tidx, const int batch_stride, const int head_stride, const int* query_table
) {
    ...
    return query_table[query_idx] * batch_stride + row_offset % HRatio * head_stride + col_offset;
}
...
// Layout reshape function. Reshape Q's layout from modes (v1, v2, k) to modes ((v2, v1), k)
template <class Shape, class Stride>
__forceinline__ __device__
auto reshape_Q_tile(Layout<Shape, Stride> l) {
    // auto mode_0 = select<1, 0>(l);
    // auto mode_1 = select<2>(l);
    return make_layout(select<1, 0>(l), select<2>(l));
}
...
inline void checkPointerLocation(const void* ptr) {
    ...
    if (attributes.type == cudaMemoryTypeDevice) {
        std::cout << "Pointer points to DEVICE memory." << std::endl;
    } else if (attributes.type == cudaMemoryTypeHost) {
        std::cout << "Pointer points to HOST memory." << std::endl;
